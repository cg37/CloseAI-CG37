...
export const useChatStore = defineStore('chatStore', {
  ...
  getters: {
    allMasks: (state): Mask[] => [...state.presetMasks, ...state.customMasks],
  },
  actions: {
    addChat(): string {
    ...
    deleteChat(id: string): void {
      const idx = this.chats.findIndex((it) => it.id === id)
      if (idx >= 0) {
    ...
    editMask(id: string, title: string, content: string): void {
      const idx = this.customMasks.findIndex((it) => it.id === id)
      if (idx >= 0) {
        this.customMasks[idx] = { id, title, content }
      }
    },
